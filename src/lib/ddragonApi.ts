export async function get_summoner_name_from_id(id: string): Promise<string> {
  const response = await fetch(
    `http://ddragon.leagueoflegends.com/cdn/${process.env.DDRAGON_PATCH}/data/en_US/summoner.json`
  );
  if (!response.ok) {
...
export async function get_augment_name_from_id(id: number | null): Promise<string | null> {
  if (id === null) {
    return null;
  }
