...
import { Rank, User, Mastery, Ratelimit, MatchHistory, MatchPlayer, Match } from '@prisma/client';
...
export async function get_ranks_by_summonerId(region: string = '', summonerId: string = ''): Promise<Rank[]> {
    var latest_ranks = await prisma.rank.findMany({
        where: { summonerId: summonerId }, distinct: ['queueType'], orderBy: {
            createdAt: 'desc',
        }
    });
    if (latest_ranks.length !== 0 && (Date.now() - latest_ranks[0].createdAt.getTime()) / 1000 < +(process.env.RANK_CACHE_IN_SECONDS || 30)) {
        return latest_ranks
    }
    ...
    var ranks = await request_riot<Rank[]>(url);
    if (ranks == null) {
        return []
    }
...
export async function get_matchhistory_by_puuid(large_region: string = '', puuid: string = ''): Promise<MatchHistory> {
    ...
    if (latest_downloaded_match != null && (Date.now() - latest_downloaded_match.updatedAt.getTime()) / 1000 < +(process.env.MATCH_HISTORY_CACHE_IN_SECONDS || 30)) {
    ...
    var index = 0
    while (true) {
        let match_history_100 = await request_riot<string[]>(url, { start: (index * 100).toString(), count: "100" });
        if (match_history_100 == null) {
            break
        }

        if (latest_downloaded_match_array.length !== 0 && latest_downloaded_match_array.some(id => match_history_100?.includes(id))) {
...
export async function get_match_by_matchId(large_region: string = '', id: string = '') {
    ...
    var match_from_api = await request_riot<any>(url, {});
...
async function request_riot<T>(url: string, params: { [name: string]: any } = {}): Promise<T | null> {
